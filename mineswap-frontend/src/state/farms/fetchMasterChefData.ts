...
const masterChefFarmCalls = async (farm: SerializedFarm) => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(quoteToken.chainId) ? ChainId.BASE_GOERLI : ChainId.BASE
...
export const fetchMasterChefData = async (farms: SerializedFarmConfig[], chainId: number): Promise<any[]> => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(chainId) ? ChainId.BASE_GOERLI : ChainId.BASE
