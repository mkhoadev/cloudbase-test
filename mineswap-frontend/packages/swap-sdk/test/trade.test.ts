...
describe('Trade', () => {
  const ETHER = Native.onChain(ChainId.BASE)
  const token0 = new Token(ChainId.BASE, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.BASE, '0x0000000000000000000000000000000000000002', 18, 't1')
  const token2 = new Token(ChainId.BASE, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(ChainId.BASE, '0x0000000000000000000000000000000000000004', 18, 't3')
  ...
  const pair_weth_0 = new Pair(
    CurrencyAmount.fromRawAmount(WNATIVE[ChainId.BASE], JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000))
  )
  ...
  it('can be constructed with ETHER as input', () => {
    const trade = new Trade(
      ...
      CurrencyAmount.fromRawAmount(Native.onChain(ChainId.BASE), JSBI.BigInt(100)),
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result[0].route.path).toEqual([WNATIVE[ChainId.BASE], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(ETHER)
      expect(result[1].route.path).toEqual([WNATIVE[ChainId.BASE], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, WNATIVE[ChainId.BASE]])
      expect(result[0].outputAmount.currency).toEqual(ETHER)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WNATIVE[ChainId.BASE]])
