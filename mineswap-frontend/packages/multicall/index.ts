...
export function createMulticall<TProvider>(provider: ({ chainId }: { chainId?: number | undefined }) => TProvider) {
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.ETHEREUMPOW) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.ETHEREUMPOW, options, provider: _provider }) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.ETHEREUMPOW, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
