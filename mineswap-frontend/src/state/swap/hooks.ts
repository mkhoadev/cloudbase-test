...
import { TCOIN, USDC } from '@pancakeswap/tokens'
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(query, native.symbol, TCOIN[chainId]?.address ?? USDC[chainId]?.address)
...
export const useLPApr = (pair?: Pair) => {
  const { data: poolData } = useSWRImmutable(
    pair && pair.chainId === ChainId.BASE ? ['LP7dApr', pair.liquidityToken.address] : null,
    async () => {
